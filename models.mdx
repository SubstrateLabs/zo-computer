...
We rigorously evaluate models to ensure we offer the leading models in each modality (with a preference for providers that are both fast and high-quality). If there's a model you'd like to see, just ask [in Discord](https://discord.gg/zocomputer).
...
<Info>
  Your selected chat model will be used for [scheduled tasks](/scheduled-tasks) and any interactions with Zo outside the
  chat.
</Info>
